...
//! (impl-plan row 5.9 §Pre-Implementation Interface Cleanup).
//!
//! The runtime exposes a single logical identity for a source-range
//! commit: [`CommitIdentity`]. The cleanup row replaced the Phase 5
//! `IdempotencyKey` (hashed string) with this struct projection so
//! sink-physical concerns no longer leak into the runtime surface.
//!
...
//!
//! The Phase 5 per-field-sensitivity assertions (`source` / `sink` /
...
/// architectural assumption so Phase 6 concurrency cannot quietly
/// shift `range` to a per-row notion.
#[test]
