//! Re-runs the Phase 5 ack-correctness invariants under the
//! pipelined `RuntimeOptions` profile (`fetch_concurrency = 8`,
//! `decode_concurrency = 4`, `max_inflight_batches = 32`,
//! `sink.max_concurrent_commits = 4`). The bulk of Phase 6's
...
//! MaybeCommitted, fence) rather than re-running every Phase 5
//! test — integration tests can't import each other, and the
//! invariants the runtime guards are structural (admission
//! ordering, ack-on-advance, retry idempotence) and hold across
//! the entire concurrent profile.
...
/// `decode_concurrency = 4`, `max_inflight_batches = 32`,
/// `sink.max_concurrent_commits = 4`. Matches the row-6.6
/// acceptance profile in
