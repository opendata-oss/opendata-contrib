...
//! `tests/support/mod.rs`. Validates the runtime's compatibility
//! surface (descriptor flow, dry-run mode, live mode + ack frontier,
//! resume regression, decoder-accepts gate, the
...
/// Resume regression. A `BufferSource` constructed
...
/// Decoder-accepts gate. Runtime must call
...
// live in `tests/ack_correctness.rs` alongside the rest of the
// INV-MAYBE-COMMITTED-RESOLVES tests.
