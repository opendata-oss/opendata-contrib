//! Ack-correctness harness. Each test pins one named `INV-*`
//! invariant.
//!
...
/// INV-MAYBE-COMMITTED-RESOLVES (Committed branch).
///
...
/// INV-MAYBE-COMMITTED-RESOLVES (Unknown branch).
///
/// `Sink::write` returns `MaybeCommitted`; `check_committed`
...
///
/// Choreography. Every `Consumer::with_object_store` calls
...
/// The `Runtime` is single-source, so this test drives
/// `AckCoordinators` directly with a synthetic register/commit
/// driver. A multi-source `Runtime` would rerun the equivalent
