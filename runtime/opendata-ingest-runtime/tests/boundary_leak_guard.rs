//! Boundary-leak guard for the runtime/sink interface.
//!
//! The runtime crate exposes
...
//! `IdempotencyKey` / `CommitGroupBatch` / `RecordSize` back from the
//! runtime crate.
//!
...
fn allowlisted(path: &Path) -> bool {
    ...
    s.ends_with("runtime/opendata-ingest-runtime/tests/boundary_leak_guard.rs")
        ...
        // never compiles and is a tracked follow-up. The guard
        // ignores it since the cfg makes the contents unreachable.
