...
//! resolution is wired in the orchestrator (`runtime.rs`); this file
...
/// before pausing upstream pulls (RFC 0002 §Backpressure Model).
...
pub trait Sink: Send + Sync + 'static {
    ...
    /// `Ok` only when the full commit is durable per RFC 0002;
