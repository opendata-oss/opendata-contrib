//! Per-entry metadata envelope (Buffer wire format).
//!
//! Producers attach a 4-byte header to each entry's metadata payload.
//! The shape is `{version, signal_type, encoding, reserved}`. RFC 0003
//! mandates per-entry decoding because a single Buffer batch can carry
//! multiple ranges with different envelopes.
//!
//! For the alpha, an envelope that doesn't match the configured
//! `(signal_type, encoding)` is a fail-closed condition: the runtime
//! halts rather than ack the offending range. A future release may
//! route per entry to per-signal pipelines instead.
//!
//! Phase 4.3 moves this module from `clickhouse-ingestor::envelope`
//! into the runtime crate. The Phase 4.2 trait surface
//! (`Decoder::accepts`) already references `MetadataEnvelope`; this
//! file replaces the 4.2 stub with the full parser/validator.
...
/// Decoded form of the 4-byte envelope. Decoders match on this via
...
pub fn decode_envelopes(batch: &SourceBatch) -> Result<Vec<MetadataEnvelope>, EnvelopeError> {
    batch
        .entries
        .iter()
...
mod tests {
    ...
    fn batch_with_envelopes(envelopes: Vec<&[u8]>) -> SourceBatch {
        SourceBatch {
            source: "test".into(),
            sequence: 1,
