//! Per-source ack coordinator (RFC 0002 §Per-Source Ack
//! Coordinator).
//!
...
//!
//! Pinned invariants (the full `INV-*` taxonomy):
//!
...
/// commit. RFC 0002 §Per-Source Ack Coordinator > State Machine: a
/// single sink-commit bit per range, no per-chunk tracking.
...
/// coordinator: the pipelined runtime keeps `register_pending` in
/// the synchronous descriptor-admission task while running
/// `mark_committed` / `advance_frontier` from the parallel post-write
/// stage on the same per-source actor.
...
/// Registry of per-source [`AckCoordinator`]s. The registry never
/// iterates across sources during a state transition
/// (INV-MULTISOURCE-ISOLATION).
...
impl AckCoordinators {
    ...
    /// Remove a coordinator from the registry and return it. The
    /// per-source actor takes ownership of its coordinator via this
    /// method so admission and completion arms can hold
...
mod tests {
    ...
    // --- Out-of-order completion (state-machine layer). ---
