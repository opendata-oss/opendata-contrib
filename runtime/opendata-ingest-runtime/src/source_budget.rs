...
//! oversize-fault gate in the decode worker; the
...
impl SourceByteBudget {
    ...
    ///
    /// Used by the multi-descriptor (K) admission extension path:
    /// after the blocking gate claims the first reservation, the
    /// admission arm tries
...
/// stage before clearing the budget. Drives per-stage
/// `runtime_stage_inflight_bytes{stage=...}` gauges without manual
/// decrement bookkeeping on every error / drop path.
