...
//!
//! Built on the RFC 0003-direct surface (`Consumer::next_descriptors`,
//! `Consumer::fetch_handle`, `Consumer::ack_through`).
//! `BufferSourceFetchHandle` wraps `Arc<buffer::ConsumerFetchHandle>`
//! and is safe to clone into N fetch worker tasks per RFC 0003
//! §Concurrency Model.
//!
...
//! descriptor twice is safe by the same RFC. The parallel fetch
//! worker pool runs against this surface.
...
/// Per-range metadata as it flows from the buffer source. The
/// byte-format envelope parser reads from `raw_metadata`.
#[derive(Debug, Clone)]
...
/// single `next_descriptors` call. The byte axis is wired end to end
/// via `SourceByteBudget`.
...
/// entries.
///
/// The `BufferSourceFetchHandle::fetch` impl calls this to convert
/// each underlying `buffer::ConsumedBatch` into a [`SourceBatch`].
...
/// The per-source actor task holds the `&mut BufferSource` for the
/// source's lifetime; admission and `ack_through` happen on distinct
/// `select!` arms inside that one task, so no synchronization wrapper
/// is needed.
pub struct BufferSource {
    ...
    /// keep [`AckFlushPolicy::EveryN`] semantics even though the
    /// buffer-side [`buffer::Consumer::ack_through`] dequeues durably
    /// on every call (RFC 0003). Without this
...
impl BufferSource {
    ...
    ///
    /// `_budget` is the byte-budget filter, wired end to end via
    /// [`crate::source_budget::SourceByteBudget`]; the buffer-side
    /// `next_descriptors` does not accept a budget arg, so the runtime
    /// gates admission on the budget BEFORE calling this method.
    ...
    /// clone this into many fetch worker tasks.
    ...
    /// runtime preserve the `AckFlushPolicy::EveryN` semantic on top
    /// of RFC 0003's durable-immediate `Consumer::ack_through`.
