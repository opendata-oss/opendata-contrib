//! OTLP per-entry metadata envelope.
//!
//! OpenData producers prepend a 4-byte header to each entry's metadata
//! payload: `{version, signal_type, encoding, reserved}`. This is a
//! producer↔decoder convention, not part of the Buffer wire format —
//! the runtime carries the metadata as opaque bytes and never inspects
//! it. This module is the OTLP decoder's parser/validator for that
//! header.
//!
//! An entry whose envelope doesn't match the decoder's configured
//! `(version, signal_type, encoding)` is a fail-closed condition: the
//! decoder returns an error and the runtime halts rather than ack the
//! offending range.
...
/// The envelope the decoder expects every entry to carry.
...
/// Decode the envelope for a single entry's opaque metadata payload.
pub fn decode_one(entry_index: u32, payload: &[u8]) -> Result<MetadataEnvelope, EnvelopeError> {
