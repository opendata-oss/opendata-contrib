...
//! (see RFC 0001 "ORDER BY Tradeoff").
//!
...
/// requires a new table plus backfill (RFC 0002 dedupe constraint).
pub fn logs_table_ddl(config: &LogsAdapterConfig) -> String {
    // Two e2e-latency columns appear after `_adapter_version`:
    //   * `_odb_gateway_received_at` is Nullable so an in-place
    //     `ALTER TABLE … ADD COLUMN` against a pre-existing table
    //     leaves its rows with NULL gateway timestamps; the harness
    //     filters those out of quantile math.
...
mod tests {
    ...
    fn ddl_carries_stage2_e2e_latency_columns() {
        ...
        assert!(
            ...
            "DDL must declare the gateway column as nullable so an in-place ALTER \
             leaves pre-existing rows with NULL gateway timestamps: {ddl}"
        );
        assert!(
