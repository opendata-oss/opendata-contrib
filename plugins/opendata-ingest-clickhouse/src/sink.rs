...
//!   runtime asks); per RFC 0002 the runtime treats
...
/// to exercise the row-drop guard.
    ...
    A: Adapter<Input = DecodedLogRecord> + Send + Sync + 'static,
{
    ...
    budget: SinkBudget,
    /// Typed metric struct. The per-commit row + byte counters emit
    /// through this Arc. `Default::default()` is used when callers
    /// don't supply one, so unit tests don't need to construct a
    /// metric struct.
    ...
    A: Adapter<Input = DecodedLogRecord> + Send + Sync + 'static,
{
    ...
    /// Explicit-metric constructor. The bin uses this so the
    ...
    /// Accessor for the typed metric surface; the per-commit row +
    /// byte counter emission reads from this.
    ...
    A: Adapter<Input = DecodedLogRecord> + Send + Sync + 'static,
{
    ...
    async fn write(&self, commit: SinkCommit) -> Result<SinkCommitResult, SinkCommitFailure> {
        ...
        // for the source range RFC 0002 §Runtime/Sink Boundary
        // specifies; fail closed if a future runtime change
        ...
        // Bucket by _odb_run_id BEFORE the adapter consumes
        // `selected`. The adapter sorts and emits
        ...
        // Row-drop contract guard: the adapter's planned chunks must
        // cover every input record. An adapter that drops rows would
        // silently advance the ack frontier past records that never
        // reached ClickHouse — the regression
        // `runtime_rejects_adapter_that_drops_rows` guards against
        // this. Fail closed before any HTTP call.
...
mod tests {
    ...
    /// `ClickHouseSink::write` must reject a sink commit where the
    /// adapter's planned row total is less than the input record
    /// count (the `runtime_rejects_adapter_that_drops_rows`
    /// regression, at the sink layer). The writer is constructed with
    /// a fake endpoint the guard never reaches (it fires before
    /// `execute_all` runs).
    #[tokio::test]
