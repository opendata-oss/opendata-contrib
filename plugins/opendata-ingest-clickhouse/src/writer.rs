...
//! `insert_quorum`. The serializer is pluggable via
//! [`ChunkSerializer`]: JSONEachRow (default) and
//! RowBinaryWithNamesAndTypes are both available.
//!
...
/// How the writer manages its HTTP client. Offers a per-call mode
/// and a pooled mode.
#[derive(Debug, Default, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "mode", rename_all = "snake_case")]
pub enum HttpClientMode {
    /// Build a fresh `reqwest::Client` per HTTP attempt. See the
    /// silent-insert-drop note in the comment on `http_client`.
    #[default]
    PerCall,
...
pub struct WriterConfig {
    ...
    /// (`INSERT ... FORMAT <X>`). Default: JSONEachRow.
    pub serialization_format: SerializationFormat,
    /// HTTP client management mode. Default: PerCall.
...
pub struct ClickHouseWriter {
    ...
    /// Typed metric struct. All writer-side emission
...
impl ClickHouseWriter {
    ...
    /// Explicit-metric constructor. Use this from the bin so
    ...
    /// Accessor for the typed metric surface; the writer's per-chunk
    /// + per-attempt emission reads from this.
    ...
    /// are dedup-safe at the table level. Emits four metric families:
    ...
    ///
    /// `clickhouse_insert_duration_seconds` **includes serialization
    /// time**. The first attempt's timer therefore wraps
