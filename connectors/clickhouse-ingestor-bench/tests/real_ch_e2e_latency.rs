//! Stage 2 — end-to-end latency column verification.
//!
//! Exercises the production decode → adapter → writer → CH → SELECT
//! path against a testcontainers ClickHouse to prove the two Stage-2
//! columns (`_odb_gateway_received_at` and `_odb_clickhouse_inserted_at`)
//! travel end-to-end:
//!
