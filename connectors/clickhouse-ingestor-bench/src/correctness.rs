...
//! `AckInvariantCheck`.
//!
//! The smoke gate: every `passed == true` AND
//! `summary.duplicate_records_post_dedupe == 0` AND every
//! `sources[s].records_missing_from_sink == 0`.
...
pub async fn run_smoke(run_dir: &Path) -> RuntimeResult<CorrectnessReport> {
    ...
    // `multi_source_ack_isolation` is the fifth named check, but
    // the runtime is single-source (multi-source `RuntimeBuilder`
    // support is future work). Running the check against a single
    // source would degenerate to self-consistency and report
    // `passed: true` without actually exercising the multi-source
    // invariant. Omitting the check keeps the artifact honest: only
    // invariants actually exercised are reported. A multi-source
    // single-sink e2e scenario would wire into `run_smoke` once the
    // runtime gains `Vec<BufferSource>` support. The
    // `run_multi_source_ack_isolation` function and
    // `CheckName::MultiSourceAckIsolation` variant stay defined so
    // that landing is a drop-in re-add, not a re-port.
...
//
// Deferred — the runtime is single-source. Kept defined so a
// future multi-source landing is a drop-in re-add, not a
...
async fn run_multi_source_ack_isolation(
    ...
    // The runtime is single-source (multi-source support is future
    // work). The structural invariant — "no source's frontier
    // advances past
