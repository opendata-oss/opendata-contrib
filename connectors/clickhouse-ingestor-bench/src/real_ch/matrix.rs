//! Matrix runner (lightweight). Sweeps a small product of
//! `(SerializationFormat, HttpClientMode)` against a shared
//! testcontainers ClickHouse. A full 6-axis grid (workload ×
//! concurrency × chunk_rows × chunk_bytes × format × http_mode) is
//! deferred; the 4-point format × http_mode sweep is enough for the
//! bottleneck classification to disambiguate `serialize_cpu_bound`
//! from `http_io_bound` vs `balanced` at the same workload.
//!
...
fn write_matrix_index(
    ...
    let matrix = json!({
        ...
        "skip_summary": {
            ...
            "notes": "Lightweight: only format × http_mode dimensions; the workload-axis correctness check from a full 6-axis sweep doesn't apply here.",
        },
    });
...
pub fn build_matrix_correctness(points: &[MatrixPointResult]) -> Value {
    ...
    json!({
        ...
        "summary": {
            ...
            "notes": "Lightweight: counts-only correctness across every point. Full `ack_invariant_checks` against the production ClickHouseSink land alongside the TestObservableSink trait refactor.",
        },
        "points": Value::Object(points_map),
    })
}
