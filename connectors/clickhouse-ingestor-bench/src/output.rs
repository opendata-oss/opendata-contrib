...
/// `correctness.json` schema v2 — Phase 6 single-service shape
/// (one `summary` + one `sources` map; the alternative
/// `services_under_test[]` shape kicks in only for the duplicated-
/// queue Phase 10 scenario).
...
/// `metadata.json` schema v2 — minimal Phase 6 smoke shape. The
