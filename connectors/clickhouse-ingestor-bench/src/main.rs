//! `clickhouse-ingestor-bench` binary. Runs the in-memory Phase 6
//! correctness smoke (Phase 6.x closeout §1.1) and writes
//! `correctness.json` + witness JSONL files under
//! `<bench-results-root>/phase06/correctness-smoke/<UTC>-phase6-smoke/`.
//!
//! Real perf-test runs against ClickHouse + S3 are operator work
//! per `plans/odb-high-throughput/local-development.md`; the
//! binary here is wired only for the smoke today.
...
#[command(
    ...
    about = "Phase 6 correctness smoke for the OpenData → ClickHouse runtime"
)]
struct Args {
