//! Matrix sweep binary. Spins up a `testcontainers` ClickHouse, runs
//! the lightweight 4-point matrix (JSONEachRow/RowBinary ×
//! PerCall/Pooled), and writes matrix.json + per-point artifact
//! bundles for the bottleneck readout.
...
#[command(
    name = "phase07-matrix",
    about = "Format × http_mode matrix sweep (testcontainers)."
)]
struct Args {
