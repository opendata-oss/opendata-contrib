//! Iteration Protocol for the stage-latency bench.
//!
//! Adapted for the in-memory dry-run shape: a fresh
//! `InMemoryFixture` per iteration (no real
...
//! drain key.
//!
//! A future refactor could extract this into a generic
//! `iteration.rs` module parameterized over a fixture trait.
...
pub struct IterationParams {
    ...
    /// Iterations whose timed window is shorter than this floor fail
    /// with a clear error. `MIN_TIMED_WINDOW_SECONDS = 20.0` is used
    /// for real-CH runs; the in-memory dry-run lets the operator
    /// lower this for tests + CI workloads that can't realistically
    /// take 20 s of in-memory pipeline work.
...
pub async fn run_iteration(
    ...
    //   Rather than prefilling the entire workload upfront, the
    //   in-memory dry-run splits the producer into a warmup phase +
    //   a timed phase. Reason: the in-memory BenchSink resolves
    //   writes in microseconds, so a single upfront prefill lets the
    //   runtime ack every payload before the bench reaches the
    //   warmup-snapshot boundary — collapsing the protocol into
    //   "everything is warmup, nothing is timed". The two-phase shape
    //   keeps the warmup boundary observable and the timed window
    //   captured cleanly for in-memory workloads, with the trade-off
    //   that the producer runs concurrently with the runtime within
    //   each phase rather than fully ahead of it. The real-CH variant
    //   pays enough sink latency per chunk that the upfront prefill
    //   works directly.
...
/// Real-CH variants set their own.
pub fn default_runtime_options() -> RuntimeOptions {
    // Drive the full pipeline, not dry-run — all four stage labels
    // (including sink_dispatch) need to emit samples. Dry-run skips
    // Sink::write and the Buffer ack/flush.
    RuntimeOptions {
        dry_run: false,
