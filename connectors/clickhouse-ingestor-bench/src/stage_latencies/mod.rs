//! Stage-latency bench.
//!
//! Drives the pipelined runtime (`Runtime::builder` + stage
//! histograms) against an in-memory `BufferSource` + `BenchSink`,
//! captures the four stage labels
//! (`source | fetch | decode | sink_dispatch`) via the process-
//! global `metrics-util::DebuggingRecorder`, and writes
//! `metadata.json` / `results.json` / `timeseries.json` per the
//! benchmark v2 schema.
//!
//! This bench has no `correctness.json` (in-memory dry-run is the
//! wrong layer for a correctness gate; the real-ClickHouse bench
//! owns that). A future refactor could extract the iteration
//! protocol into a generic `iteration.rs` driver parameterized over
//! a fixture trait and share the scaffolding here.
...
impl Default for StageLatenciesConfig {
    fn default() -> Self {
        Self {
            ...
            timed_payloads: 32,
            iterations: 3,
            // In-memory: 0.0 disables the floor entirely.
            // Real-CH runs raise this to 20.0.
...
pub async fn run_stage_latencies(cfg: StageLatenciesConfig) -> Result<RunArtifacts> {
    ...
    let inputs = RunInputs {
        ...
        unit_id: "7.1",
        unit_title: "Stage-latency bench on Runtime::builder + stage histograms",
