...
//!
//! Stage-1 dual-backend rendering (see
//! `plans/odb-high-throughput/stage1-metrics-migration-plan.md`): the
//! response concatenates the `prometheus-client` Registry encode (the
//! typed runtime + clickhouse metric structs introduced in Stage 1)
//! and the legacy `metrics-rs` `PrometheusHandle::render()` output
//! (still load-bearing for the buffer crate's emissions). Both halves
//! are valid Prometheus text format; the scrape consumer sees one
//! unified document.
//!
